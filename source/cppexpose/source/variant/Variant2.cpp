...
std::string Variant2::toString() const
{
    if (m_value) return m_value->toString();
    else         return "";
}

bool Variant2::fromString(const std::string & value)
{
    if (m_value) return m_value->fromString(value);
    else         return false;
}

bool Variant2::toBool() const
{
    if (m_value) return m_value->toBool();
    else         return false;
}

bool Variant2::fromBool(bool value)
{
    if (m_value) return m_value->fromBool(value);
    else         return false;
}

long long Variant2::toLongLong() const
{
    if (m_value) return m_value->toLongLong();
    else         return 0ll;
}

bool Variant2::fromLongLong(long long value)
{
    if (m_value) return m_value->fromLongLong(value);
    else         return false;
}

unsigned long long Variant2::toULongLong() const
{
    if (m_value) return m_value->toULongLong();
    else         return 0ull;
}

bool Variant2::fromULongLong(unsigned long long value)
{
    if (m_value) return m_value->fromULongLong(value);
    else         return false;
}

double Variant2::toDouble() const
{
    if (m_value) return m_value->toDouble();
    else         return 0.0;
}

bool Variant2::fromDouble(double value)
{
