...
std::string JSON::stringify(const Variant & root, JSON::OutputMode outputMode)
{
    ...
    return stringify(root, beautify, "");
}
...
bool JSON::readDocument(Variant & root, Tokenizer & tokenizer)
{
...
bool JSON::readObject(Variant & root, Tokenizer & tokenizer)
{
    // Create object
    root = Variant::map();
...
bool JSON::readArray(Variant & root, Tokenizer & tokenizer)
{
    // Create array
    root = Variant::array();
...
bool JSON::readValue(Variant & value, Tokenizer::Token & token, Tokenizer & tokenizer)
{
    if (token.content == "{")
    {
...
std::string JSON::stringify(const Variant & root, bool beautify, const std::string & indent)
{
    ...
    if (root.isVariantMap()) {
        ...
        for (auto it : *root.asMap()) {
            ...
            if (var.isVariantMap() || var.isVariantArray()) {
                ...
                value = escapeString(stringify(var, beautify, ""));
